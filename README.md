...
A native, all-in-one Android port is available in [`android-app/`](android-app/README.md). It includes provider-agnostic AI chat, encrypted credentials, persistent memory, speech input/output, an animated agent face, and on-device camera gesture recognition.
